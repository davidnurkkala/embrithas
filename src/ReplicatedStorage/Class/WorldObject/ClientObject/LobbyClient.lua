...
function LobbyClient:ShowMissions()
	if self.MissionSelectGui then return end
	...
	self.MissionSelectGui = self:CreateNew"MissionSelectGui"{}

	if self.IsTutorial then
		spawn(function() self:OnTutorialUpdated("explainMissions") end)
	end
end
...
function LobbyClient:StartHidingMissions()
...
function LobbyClient:HideMissions()	
	if self.MissionSelectGui then
		self.MissionSelectGui:Close()
		self.MissionSelectGui = nil
	else
		return
	end
...
function LobbyClient:OnTutorialUpdated(step)
	...
	elseif step == "explainMissions" then
		...
		self.D = self:Dialogue("Here's where you'll see the missions you can embark on. Some are locked because you aren't experienced or haven't completed the required missions. For now, let's take a look at \"Recommended Missions.\" These are always picked for your level of experience.", leon{ManualTiming = true, Parent = self.LobbyGui, Scale = scale, Position = "Bottom"})
		self.MissionExamined:Wait()
		self.D:End()

		self.D = self:Dialogue("Now select the mission \"Rookie's Grave.\" Don't worry about the name.", leon{ManualTiming = true, Parent = self.LobbyGui, Scale = scale, Position = "Top"})
		self.MissionSelected:Wait()
		self.D:End()

		self.D = self:Dialogue("Under normal circumstances, you could form a party now. However, I think it's best if you take on Rookie's Grave by yourself to start. Go ahead and click \"Select Mission.\"", leon{ManualTiming = true, Parent = self.LobbyGui, Scale = scale, Position = "Top"})
	end
end
